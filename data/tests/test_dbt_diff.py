...
def test_occurrences_schema_matches():
    """Column names AND types from DESCRIBE match exactly between sandbox and public (40 cols).

    Asserts the full ordered list of (column_name, data_type) pairs is identical.
    Verified baseline: 40 columns with identical names and types in both files
    (Phase 160 dropped the scalar `place_slug` — 37→36 — moving place membership to
    the separate `occurrence_places` bridge; later phases re-grew it to 39;
    beeatlas-sn8 added the DEM-derived `elevation_dem_m` — 39→40). NOTE: an intentional occurrences-contract
