...
# (test_run_py_integration removed with run.py: the orchestrator is now the
# Stelis graph, where the feeds edge is pinned by stelis/src/plan-test.rkt —
# and where feeds correctly does NOT depend on species-export; that ordering
# was an artifact of run.py's linear STEPS list. Cross-repo contract
# verification is st-whm.)
