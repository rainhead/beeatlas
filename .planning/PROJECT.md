...
---
*Last updated: 2026-06-08 after Phase 136 (Deduplication) completion under resumed v4.7 (Checklist Records as Point Data) — DUP-01..03 verified (7/7 must-haves), build-time checklist dedup with conservative curator-gated suppression now wired into the pipeline (`dedup-candidates`→`dedup-gate` STEPS). Phases 135–136 complete; Phases 137 (Promotion into Occurrences) and 138 (Frontend Points & Detail Card) remain. v4.8 (Fast, Honest Test Suite) shipped 2026-06-08 (17/17). Backlog: Phase 999.1 (debounce URL updates on map move), 999.2 (dependabot). Next: `/gsd:plan-phase 137`, or `/gsd:resume-work` for v4.7.*
