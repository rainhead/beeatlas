...
## Milestone: v7.0 Species Trait Annotations — COMPLETE (2026-06-30)

**Shipped:** Curated ecological trait annotations end-to-end (Phases 173–174, merged via PR #39). **173** (ad-hoc) — a `species_traits` dbt mart keyed on `canonical_name` assembling sociality / nesting / diet breadth + host plant / native status / cuckoo host bee from three license-clean seeds (USGS Bee-Gap 2017 PD, Fowler & Droege specialists, a genus-level backbone); every label carries a `*_source` provenance column and routes its join key through `int_synonyms`; also fixed a latent synonymy gap in the checklist/ecdysis arms. **174** — surfaced the traits via the Path-B `species.json` merge (no `species.parquet`/contract change, no committed artifacts): a "Traits" definition list on the species detail page (linked cleptoparasite hosts, native `title=` provenance tooltips, friendly domain labels) and compact sociality + Specialist badges on the index tree, genus, and subgenus pages — build-time Nunjucks, zero JS. All TRAIT-DATA-01..03 + TRAIT-UI-01..05 satisfied.

**Notable:** Post-merge review surfaced the host plant for the 44% of Fowler specialists carrying only a genus-level host (now "Specialist (Salix)" etc.), alphabetized the genus/subgenus species lists, and restructured the species detail hero into a 2×2 grid. One display issue deferred (`checklist_count=0` vs `on_checklist=true` — todo filed). Operator action pending: one-time `SKIP_INTEGRATION_GATE=1 bash data/nightly.sh` on maderas to land the new-shape `species.json` in S3.

## Milestone: v6.0 My Work — Progress & Provenance — COMPLETE (2026-06-28)

**Shipped:** The first **"work" surface** — a bookmarkable, no-auth, public per-collector page (`/collectors/{login}/`) showing a volunteer's collection→ID lifecycle as an event stream and their accomplishments as coverage/breadth, on a rebuilt occurrence model (7 phases, 167–172 incl. inserted 171.1, 16 plans, 2026-06-25→28). **167** — a unified `collector_inat_login` COALESCE column projected through the occurrences mart across all 5 `int_combined` arms (host-first priority: `host_inat_login` > `specimen_inat_login` > `user_login`, so the sample owner wins over a third-party specimen-photo poster; `assert_collector_prefers_host` test); dbt contract 36→37. **168** — a single `id_date VARCHAR` column (dirty-parsed Ecdysis `date_identified`) so a specimen reads as a two-event **Collected→Identified** timeline (37→38); `posted_date`/iNat `created_at` deliberately dropped (D-02: posting is not an event). **169** — 124 Eleventy `/collectors/{login}/` pages (headline stats, species-level pending-vs-identified split, `?collectors=` map deep-link) + `/collectors.html` roster. **170** — replaced the mutually-exclusive `source` enum with orthogonal `tier`(atlas/other)+`record_type` facets across all three `occ_id`-coupled consumers (`src/occurrence.ts`, `src/filter.ts`, `occurrence_places.sql`) in one atomic commit with a positional-coupling Vitest assertion; tier-driven filter/symbology/card; `tier=` URL with legacy `src=` back-compat; `inat_obs`→`inat_expert`; contract 38→39. **171** — a reverse-chronological Collected→Identified `<table>` event feed (rank-aware links, per-collector Atom feed, paginated sub-pages), waba_specimen cataloguing as one continuous row. **171.1** — moved the 29 MB+ collector data artifacts off git onto the `species.json` S3+manifest+`deploy.yml` fetch pattern (fixes a 171 committed-artifact defect). **172** — county/ecoregion coverage SVG maps, taxonomic-breadth species list, and an "active since YYYY (N seasons)" badge, all over the `tier='atlas'` facet; maps delivered as two committed shared base-map partials highlighted per-collector via CSS (not 248 per-collector SVGs). All 17 v1 requirements satisfied; cross-phase integration clean (5/5 seams).

**Notable:** Two collector-identity corrections landed *after* the phases closed but before milestone close — `collector_inat_login` flipped to host-first COALESCE (the sample owner, not whoever posted the specimen photo) and `display_name` switched to the most-recent `recordedBy` (not `MIN`), both saved to memory. The audit ran `tech_debt` (no blockers): the only real-world unknown is operational — the data leg (dbt 37→38→39 cols) lands in live S3 via the operator nightly on maderas. The nightly is unblocked (Phase 163 Ecdysis-auth was resolved 2026-06-24, before these contract bumps landed; the milestone close initially mis-tracked 163 as open from a stale STATE entry), so the data leg is very likely live — worth a one-time prod confirm. The `source`→facets rebuild (170) is the substrate the rest of the milestone — and future "work"/"learning" surfaces — build on.
...
**Notable:** 158 demonstrated that the right answer to a "build a match strategy" roadmap item was *curation tooling + a documented operator process*, not pipeline automation. 159's full discuss→plan→execute→verify chain ran autonomously in one session, with the verifier's `human_needed` items closed by headless Playwright UAT (`o=<ids>&pane=list` populates the sidebar from the data layer, no map-canvas interaction needed) and two accessibility findings (keyboard activation, focus visibility) caught at code review and fixed inline.

## Milestone: v5.0 Offline Field Mode — COMPLETE (2026-06-21)

**Shipped:** An installable PWA, dogfooded behind the unlisted `/app` route, that a collector with no signal can use in the field — map + table + selection running entirely against cached client-side data (8 phases, 147–154, 22 plans, ~11 days). A correctly-scoped service worker (`scope:'/app'`, no SW on `/`) precaches the app shell (vite-plugin-pwa injectManifest) and runtime-caches `occurrences.db` (~23 MB) + GeoJSON `CacheFirst` for a full offline cold-start, with reconnect re-prime, `QuotaExceededError`/`persist()` handling, a workbox-window prompt-to-reload update lifecycle (no `skipWaiting`), a "Data as of `<generation date>`" freshness label, and an online/offline indicator. Installability: `manifest.webmanifest` + a from-scratch bee-glyph icon set, Android `beforeinstallprompt` surfaced as a quiet Install affordance, iOS A2HS instructions. Field location: an offline-safe Mapbox `GeolocateControl` (state owned by `<bee-atlas>`, control hosted by `<bee-map>` — invariant preserved) plus "occurrences near me" (GPS → ~10 km shareable spatial **filter** reusing the shift-drag bounds mechanism).

**Notable:** Phase 154 was reshaped mid-flight by a ToS review — caching Mapbox basemap tiles for *offline* use is **not licensed for the web SDK** (Product Terms 2026-06-17 §1.9/§2.8.1; offline is Mobile-SDK-only). It shipped instead as a ToS-compliant `StaleWhileRevalidate` performance cache (token retained, ≤30-day TTL, telemetry/billing excluded), with the full legal analysis recorded in `docs/adr/0001-mapbox-basemap-cache.md` (the project's first ADR) — the original "cached tiles offline, self-test only" target was retired as unlicensable. Offline basemap remains gracefully degraded (Phase 149). Two backlog items shipped in the same window: 999.1 (desktop shift-drag hint) and 999.8 (separated the spatial-bounds FILTER from per-record SELECTION).

## Milestone: v4.10 Housekeeping — COMPLETE (2026-06-09)

**Shipped:** Two independent maintenance/polish items promoted from backlog, no formal requirements. **Phase 145 (Dependabot):** the `.github/dependabot.yml` v2 config — previously github-actions only — now covers all dependency ecosystems: npm at `/` (root) and `/infra` (the AWS CDK project), and Python via the native `uv` ecosystem at `/data`, each on a weekly schedule grouping minor+patch into one PR while leaving major bumps as individual PRs (the github-actions entry was retrofitted with the same grouping). **Phase 146 (debounce URL):** discussion reframed the goal — a 500 ms history debounce already existed; the real problem was that viewport `pushState` was the app's *only* history-entry source, so every deliberate pan/zoom polluted the back button. Replaced it with session-coalescing via a `_viewportSessionActive` flag in `<bee-atlas>`: the first settled move of an exploration session pushes one entry, subsequent moves `replaceState` onto it, and any non-viewport state change (filter/selection/UI) resets the flag — so a whole exploration is one back-button entry. The v4.9 map-init invariants (`_filterResolving`, `_isRestoringFromHistory`) were preserved with explicit no-regression tests.

**Notable:** the code-review gate again earned its keep — on Phase 146 it caught a sibling test file (`bee-atlas-legacy-taxon.test.ts`) still calling the renamed `_pushUrlStateDebounced()`, a red-suite regression the scoped per-plan test run structurally could not see; on Phase 145 it caught the uncovered `infra/` npm project, surfacing decision D-06. One deferred item: Phase 145's Dependabot composite-action coverage (WR-02) is only observable after the first weekly GitHub run (tracked in `145-HUMAN-UAT.md`).

## Milestone: v4.9 Map-Init Readiness — COMPLETE (2026-06-09)

**Shipped:** The recurring map-init race class — a legacy-taxon deep link stranding the URL or flashing unfiltered dots while the taxon name resolved async — was retired *structurally* rather than timed-around, building on the `ready.ts` readiness barriers (`taxaReady`/`mapReady`) from quick task 260608-tnc. Legacy-taxon resolution collapsed from a store-and-poll dance (`_pendingLegacyTaxon` stored in `firstUpdated`, re-checked in `_loadSummaryFromSQLite`, re-stored in `_resolveLegacyTaxon`) into one linear `await taxaReady` path. A single `intendedFilterActive` getter — `isFilterActive(_filterState) || _filterResolving`, backed by a dedicated reactive `_filterResolving` `@state` flag — now governs both the hide-all guard and URL-write suppression, eliminating the second source of "are we mid-resolve" truth. The occurrence-layer render decision moved into `<bee-map>` as a pure function `f(filteredGeoJSON, intendedFilterActive)` gated on the map load lifecycle, so `<bee-atlas>` no longer pre-seeds empty collections as the hide-all mechanism — an unfiltered flash is now structurally impossible. `<bee-map>` stays a pure presenter (input-only `@property`); the `_filterQueryGeneration` race guard and style-cache bypass rules are unregressed. Phase 144, 2 plans, verified 5/5.

**Notable:** the code-review gate earned its keep — it caught a barrier-stranding regression the new architecture introduced: `markTaxaReady()` sat inside `_loadSummaryFromSQLite`'s `try` after the empty-DB early-return and before the `catch`, so both failure paths skipped it; with a legacy-taxon URL pending, `_filterResolving` would stick `true` and render the map empty forever with no error surfaced. Fixed before close by moving `markTaxaReady()` into `finally` (idempotent) and backing `_filterResolving` with `@state` (its propagation to `<bee-map>` had been working only via incidental co-mutation of other reactive fields), plus regression tests for both failure paths. One review warning (CSV-export `rows[0]`-derived headers) and three info items were deferred to `.planning/todos/pending/144-code-review-deferred.md`.
...
## Milestone: v4.7 Checklist Records as Point Data — COMPLETE (2026-06-08)

**Shipped:** The original 50,646-row Bartholomew et al. 2024 checklist CSV — its coordinates, full dates, collector, and locality discarded by Phases 76/112 — was committed as a git-LFS source and promoted into `occurrences.parquet` as a `source='checklist'` point peer, reversing the Phase 111 "checklist out of occurrences" lock (whose "lacks GPS coordinates" rationale was factually void). Ingest loads all columns with Python-side coordinate validation (NULL/`0,0`/swapped/out-of-WA-bbox excluded) and `dateparser` date normalization into a `date_quality` enum (full/year_only/none). A tiered name resolver (exact canonical → committed synonym seed → one-time build-time GBIF) keys every record to an iNat `taxon_id` via a committed audit CSV, with slash-compound LCA resolution, a `rapidfuzz` human-review gate (never auto-applied), unification onto the single dbt synonym subsystem, a within-Anthophila homonym guard, and **zero nightly taxonomy network calls**. Deduplication collapses the 5,184 internal-duplicate groups (`collapsed_count`) and conservatively flags cross-source Ecdysis pairs (exact name + full-precision date + ~1 km + collector match, NULL-ineligible) into an auditable CSV that suppresses nothing without curator sign-off (false-split preferred over false-merge). 19,929 deduped coord-bearing records entered `int_combined` as ARM 4; the dbt contract went 33→34→37 (`checklist_id`, then `verbatim_name`/`locality`/`collapsed_count`); the Phase 111 isolation test was retired to a positive floor; and `_GEO_COLS` index 7 ↔ `features.ts` `checklist:<N>` decode shipped in one atomic commit. The frontend renders checklist as distinct green (`#2c7a2c`) points keyed on `source` — clustering and `taxon_id`-filtering with the other three sources, a real `VALID_SOURCES` member with `src=checklist` URL round-trip — and the county-fill layer plus the entire `_showChecklist` chain was removed; the detail card shows accepted name + inline `(det. as {verbatim})`, collector, precision-aware Roman date, locality, collapsed-count, and Bartholomew attribution. `checklist_count` was re-sourced from the deduped status (Bombus mixtus 4,095→1,413). 21/21 requirements complete; audit passed.

**Notable:** the two credibility-critical failure modes for a scientific atlas — taxonomic over-matching and dedup false-merge — were each gated behind a committed audit CSV and curator sign-off. Shipped out of roadmap order: paused mid-Phase-135 so v4.8 (Fast, Honest Test Suite) could fix the slow/red `data/` pytest suite impeding iteration, then resumed and completed. At close, the pre-close audit surfaced 26 "open" items that proved to be 24 quick-task scanner false-positives (a `gsd-sdk` `audit-open.ts` bug that ignores the `${id}-SUMMARY.md` convention) plus 2 already-accepted UATs — all verified complete and normalized.
...
**Explicitly deferred (candidates for the next milestone):** community/shared liveness feed → `collection-event-coordination` seed (needs the append-only history table); per-identification enrichment ("IDed by X on date") via an extra nightly iNat identifications fetch; "where to go next" planning surface (gaps × access × bloom) → `where-to-go-next` seed; accomplishment depth (collector dot map, year-over-year); role badges (need a roster/identity source not in the pipeline).
...
**Open backlog (ROADMAP.md `## Backlog`):** 999.11 (federal wilderness areas as regions), 999.7 (Safari private-browsing in offline-ready UI). Promote with `/gsd-review-backlog`.

**Open phases carried forward (promoted from backlog, not milestone-assigned):** Phase 166 (seasonality charts on species/genus pages — needs a per-taxon page route that doesn't exist yet). Phases 163 (Ecdysis auth — nightly now unblocked), 164, and 165 completed 2026-06-24.
...
- ✓ **IDENT-01 / TEMP-01..02 / PAGE-01..04 / PROV-01..03 / STREAM-01..03 / ACCOM-01..04** (17/17): the first public, no-auth per-collector "work" surface on a rebuilt occurrence model. **Data model** — a unified `collector_inat_login` COALESCE column (host-first: sample owner over specimen-photo poster) + a lifecycle `id_date` (Collected→Identified two-event timeline) projected into the occurrences mart (dbt contract 36→39); the mutually-exclusive `source` enum replaced by orthogonal `tier`(atlas/other)+`record_type` facets across all three `occ_id`-coupled consumers in one atomic commit (positional-coupling Vitest assertion), tier-driven map filter/symbology/detail-card, `tier=` URL with legacy `src=` back-compat, `inat_obs`→`inat_expert`. **Pages** — 124 bookmarkable `/collectors/{login}/` Eleventy pages: headline specimens/samples/species stats, species-level pending-vs-identified status split, `?collectors=` map deep-link, `/collectors.html` roster. **Event stream** — reverse-chronological Collected→Identified `<table>` feed (full re-determination history, rank-aware BeeAtlas+iNat links, per-collector Atom feed, paginated sub-pages for 500+ records), waba_specimen cataloguing as one continuous row. **Accomplishments** — county + ecoregion coverage SVG maps, taxonomic-breadth species list, "active since YYYY (N seasons)" badge, all over the `tier='atlas'` facet. Collector data delivered via the `species.json` S3+manifest+`deploy.yml` fetch pattern (not committed artifacts). TEMP-01's `posted_date`/`created_at` intentionally dropped (posting is not an event). — v6.0 (Phases 167–172 incl. 171.1)
- ✓ **ROUTE-01..03 / OFF-01..05 / CACHE-01..05 / PWA-01..03 / LOC-01..03 / NEAR-01..03**: installable offline PWA behind unlisted `/app` — scoped pass-through SW (no SW on `/`), app-shell precache + `/data/` `CacheFirst` runtime cache with full offline cold-start, reconnect re-prime, quota/persist handling; cache-health + "Data as of `<date>`" freshness UX with prompt-to-reload updates (no `skipWaiting`); manifest + icons + Android/iOS install affordances; offline-safe GeolocateControl with `<bee-atlas>`-owned location state; "occurrences near me" as a shareable ~10 km bounds filter reusing the shift-drag mechanism — v5.0 (Phases 147–153)
- ✓ **TILE-01..02**: ToS-compliant Mapbox basemap **performance** cache — ship-enabled `StaleWhileRevalidate` `mapbox-basemap` route (token retained, 200-only, 7-day TTL, `maxEntries` 150, telemetry + `/map-sessions/` billing path excluded); re-scoped from the original offline-tile target after a ToS review found web-SDK offline serving unlicensed (§1.9/§2.8.1), with the legal analysis + compliance checklist in `docs/adr/0001-mapbox-basemap-cache.md` — v5.0 (Phase 154)
...
- ✓ **RCN-01..07**: tiered checklist name resolver — authority-strip + whitespace/case normalization to canonical binomial; exact canonical → committed synonym seed → one-time build-time GBIF tiers with iNat `taxon_id` as the terminal key and a committed `checklist_name_resolution_audit.csv`; slash-compound determinations resolve to the lowest-common-ancestor `taxon_id` via `lineage_path`; `rapidfuzz` low-confidence candidates surfaced to a human-review CSV (never auto-applied); unified onto the single dbt synonym subsystem (`occurrence_synonyms`/`int_synonyms`, disjoint Python path retired); within-Anthophila homonym guard hard-fails the build; **zero nightly taxonomy network calls** — v4.7 (Phase 135)
- ✓ **ING-01..03**: full-fidelity Bartholomew CSV (50,646 rows) committed as a git-LFS source and loaded by `checklist_pipeline.py` with `lat`/`lon`/`date`/`recordedBy`/`locality`/`verbatim_name`; Python coordinate validation excludes NULL / `0,0` / lat-lon-swapped / outside-WA-bbox from the point arm (excluded count logged); mixed/missing dates (ISO, `m/d/yyyy`, year-range, pre-1900, ~13% null) normalized via `dateparser` into a date + `date_quality` enum (full/year_only/none) — v4.7 (Phase 134)
...
| Map-init race retired with named readiness barriers + single `intendedFilterActive` gate (not timers/ordering) | The legacy-taxon strand and unfiltered-flash were a recurring race class; an `await taxaReady` path + one reactive gate make the bad states unreachable by construction rather than timed-around | ✓ Good — v4.9 Phase 144; `ready.ts` barriers, `_filterResolving` `@state`, render = f(intent) in `<bee-map>` |
| `markTaxaReady()` in `finally`, not on the happy path | A readiness barrier coupled to visibility MUST always resolve; firing it only on success let the empty-DB/error paths strand `_filterResolving=true` and hang the map (caught at code review) | ✓ Good — v4.9 Phase 144; idempotent resolve + regression tests for both failure paths |
