...
---
*Last updated: 2026-06-08 after v4.8 (Fast, Honest Test Suite) milestone close — `data/` pytest suite now a green < 5 min fast tier (197 passed/9 skipped/18.8s, randomized-order-stable) with an opt-in slow tier in nightly.sh + a CI gate; 17/17 requirements complete. No active milestone. Resumable next focus: v4.7 (Checklist Records as Point Data), paused at Phase 135. Backlog: Phase 999.1 (debounce URL updates on map move), 999.2 (dependabot). Next: `/gsd:resume-work` for v4.7, or `/gsd:new-milestone`.*
