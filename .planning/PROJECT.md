...
v5.0 Offline Field Mode shipped 2026-06-21. No active milestone — start the next with `/gsd-new-milestone`.
...
**Open backlog (ROADMAP.md `## Backlog`):** 999.2 (WDFW wildlife areas as places), 999.3 (specific hikes as places), 999.4 (Regions dropdown obscured by filter button), 999.5 (capture specimen photos from non-WABA-field iNat users), 999.6 (filter by taxon from occurrence summary), 999.7 (Safari private-browsing in offline-ready UI). Promote with `/gsd-review-backlog`.
...
---
*Last updated: 2026-06-21 — closed milestone **v5.0 Offline Field Mode** (Phases 147–154, 22 plans): installable offline PWA behind unlisted `/app` — scoped SW, app-shell + `/data/` offline caching with cold-start, cache-health/freshness UX, manifest + install affordances, GeolocateControl + "occurrences near me", and a ToS-compliant Mapbox basemap performance cache (offline-tile target retired as unlicensable; see `docs/adr/0001`). Archived to `.planning/milestones/v5.0-*`; tagged `v5.0`. No active milestone — next via `/gsd-new-milestone`; open backlog 999.2–999.7. At close, resolved the stale 153 HUMAN-UAT and 145 verification statuses (Dependabot confirmed live via PRs #28–35). Carried-forward deferred item: `144-code-review-deferred.md` (low). Previously: 2026-06-10 opened v5.0; 2026-06-09 closed v4.10 Housekeeping.*
