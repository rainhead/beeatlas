...
**Goal:** Cut over from `export.py` + ad-hoc Python transforms to `data/dbt/` as the production pipeline. After v3.4, `dbt build` is the canonical way to produce `occurrences.parquet`, `counties.geojson`, `ecoregions.geojson`, `species.json`, and `species_counts.*`.

**Outcome:** 15/15 requirements satisfied across 4 phases (85–88), 14 plans, 27 tasks. Phase 85 resolved the v3.3 awkward-fit tests (TEST-01/02) and shipped the 33→30 column drop (CLEAN-02); CLEAN-01 was satisfied via documented override D-03 (FORMAT CSV macro retained with three-section rationale — no working alternative). Phase 86 ported species_export to a dbt mart + Python post-step (PORT-01), the occurrence-links join (PORT-02), and the taxon-lineage walks with LIN-05 enforcement (PORT-03); the ingestion-boundary document (PORT-04) codifies that HTTP API work stays in Python while pure SQL transforms move to dbt. Phase 87 experimented with `materialized='incremental'` on `int_combined` and recorded "keep full rebuilds" as the locked recommendation (upstream issue #74 plus < 30% wall-clock improvement). Phase 88 rewrote `data/run.py` with `_run_dbt_build` (subprocess + 3-artifact copy), deleted `data/export.py` and `_apply_migrations`, retired `scripts/validate-schema.mjs` end-to-end (file + `package.json` + `.github/workflows/deploy.yml`), confirmed `data/nightly.sh` requires no functional changes, and ran the manual frontend smoke (map / filters / table / species page all green against dbt-produced parquet). See `.planning/milestones/v3.4-ROADMAP.md`, `.planning/milestones/v3.4-REQUIREMENTS.md`, and `.planning/milestones/v3.4-MILESTONE-AUDIT.md`.

## Previous Milestone: v3.3 dbt Spike (shipped 2026-05-13)

**Goal:** Learn whether `dbt-duckdb` is the right shape for the BeeAtlas data layer by porting one representative slice end-to-end on a branch and producing a go/no-go writeup.

**Outcome:** GO-WITH-CONDITIONS. 23-model dbt project at `data/dbt/` produces `occurrences.parquet` (47,883 rows) byte-comparable to `export.py`'s output; 11/11 diff tests green; 33-column enforced contract with verified compile-time drift detection; 5-prerequisite checklist for v3.4 captured in `.planning/research/dbt-spike-findings.md`. No production surfaces touched. See `.planning/milestones/v3.3-ROADMAP.md` for phase detail and `.planning/milestones/v3.3-REQUIREMENTS.md` for the 18 satisfied requirements.
...
- ✓ Eleventy outer build wrapper: Eleventy 3.1.5 + `@11ty/eleventy-plugin-vite` 7.1.1 wrap the Vite SPA at the repo root; `frontend/` collapsed to repo root (single-package layout); `eleventy.config.js` wires Eleventy → Vite via `_site/` output; CI `deploy.yml` updated for `_site/` artifact paths; `npm run dev` from repo root with HMR; SPA URL unchanged (`/`); 172 Vitest tests green — v3.1 (Phase 74)
- ✓ Eleventy authoring scaffold: two-layer Nunjucks layout chain (`_layouts/base.njk` + `_layouts/default.njk`) via Eleventy front-matter `layout:` + `{{ content | safe }}`; `<bee-header>` Lit component embedded in `default.njk` via side-effect Vite entry (`src/entries/bee-header.ts`); orphan verification page at `/_scaffold-check/` ships permanently as deploy diagnostic; `_data/build.js` build-time metadata pattern; bee-header bundle 8,474 B gzipped — v3.1 (Phase 75)
- ✓ dbt slice ported end-to-end with 33-column contract on `marts/occurrences`; 11/11 diff tests against `export.py` output; 23-model dbt project at `data/dbt/`; GO-WITH-CONDITIONS verdict with 5-prerequisite checklist for v3.4 — v3.3 (Phases 83–84)
- ✓ TEST-01: `stg_inat__observations.id not_null` awkward-fit resolved by `WHERE id IS NOT NULL` filter; `not_null` and `unique` tests both pass on 10,845 rows — v3.4 (Phase 85)
- ✓ TEST-02: `int_ecdysis_base.ecdysis_id` cross-type `relationships` ERROR replaced with custom singular test (`test_ecdysis_id_references_source.sql`) joining via `stg_ecdysis__occurrences.id` — v3.4 (Phase 85)
- ✓ CLEAN-01: `emit_feature_collection` `FORMAT CSV` macro retained with three-section rationale (FORMAT JSON wraps scalar incorrectly, FORMAT GDAL adds incompatible `name` key, FORMAT CSV emits raw VARCHAR verbatim — accepted via documented override D-03) — v3.4 (Phase 85)
- ✓ CLEAN-02: 33→30 column drop on `marts/occurrences` contract (`specimen_inat_login`, `specimen_inat_family`, `specimen_inat_genus` removed); `src/sqlite.ts` CREATE TABLE updated; `test_dbt_diff.py` schema assertion updated — v3.4 (Phase 85)
- ✓ PORT-01: `species_export.py` rewritten as thin dbt-mart consumer — reads 18-col `sandbox/species.parquet`, appends slug via `feeds._slugify`, emits 19-col `species.parquet` + byte-comparable `species.json` + `seasonality.json` — v3.4 (Phase 86)
- ✓ PORT-02: occurrence-links join expressed in dbt — `stg_ecdysis__occurrence_links` source + `int_ecdysis_base` LEFT JOIN + `int_waba_link` computes `specimen_observation_id`; Python HTML scraping retained in `ecdysis_pipeline.load_links` per ingestion-boundary doc — v3.4 (Phase 86)
- ✓ PORT-03: taxon-lineage in dbt — `stg_inat__canonical_to_taxon_id` + `stg_inat__taxon_lineage_extended` sources + `test_lin05_lineage_coverage.sql` singular test (735/735 = 100% coverage, ≥0.95 threshold enforced) — v3.4 (Phase 86)
- ✓ PORT-04: ingestion-boundary doc records `resolve_taxon_ids.py` stays in Python (HTTP API, rate limiting, UPSERT, CSV side effects); dbt source seam declared in `sources.yml` — v3.4 (Phase 86)
- ✓ VALIDATE-01: `test_dbt_diff.py` extended with 5 SKIP-guarded species diff tests (Wave 0); harness stays green throughout milestone — v3.4 (Phase 86)
- ✓ TEST-03: `materialized='incremental'` experimented on `int_combined` (ARM 1 watermark + ARM 2 `AND FALSE` skip); 4 timed runs; recommendation locked as "keep full rebuilds" (dbt-duckdb does not support incremental + external materializations per upstream issue #74; observed wall-clock improvement < 30% threshold; ARM 2 complexity tax) — v3.4 (Phase 87)
- ✓ CUTOVER-01: `data/run.py` invokes dbt build via `_run_dbt_build` (`subprocess.run(["bash", str(_DBT_SCRIPT), "build"], check=True)` + `shutil.copy2` of 3 artifacts to `EXPORT_DIR`); `data/export.py` deleted; `data/species_export.py` retained as non-transform post-step; non-zero exit propagates via `CalledProcessError` — v3.4 (Phase 88)
- ✓ CUTOVER-02: `_apply_migrations()` deleted from `data/run.py`; Phase 47 `geom GEOMETRY` and Phase 48 `host_observation_id` rename invariants mapped to dbt source contracts and staging models in `088-CUTOVER-LOG.md ## Migration Mapping` with file:line citations — v3.4 (Phase 88)
- ✓ CUTOVER-03: `scripts/validate-schema.mjs` deleted; `validate-schema` removed from `package.json` build chain; `.github/workflows/deploy.yml` step removed; `CLAUDE.md` updated to name the dbt 30-column contract as the canonical schema gate; `npm run build` exits 0 — v3.4 (Phase 88)
- ✓ CUTOVER-04: `data/nightly.sh` confirmed correct (no functional changes needed); `set -euo pipefail` + `uv run python run.py` + 3-artifact S3 upload + recursive `feeds/` upload all intact; no `--exclude` or `--full-refresh` flags needed — v3.4 (Phase 88)
- ✓ VALIDATE-02: end-to-end frontend smoke approved 2026-05-14 (map renders, filters work, table populates, species page works) against dbt-produced parquet with zero console errors; assertive companion `test_occurrences_schema_matches` PASSES post-cutover — v3.4 (Phase 88)
...
- `data/stub_handler.py` + Lambda CDK artifacts dormant — Lambda is not the execution path; maderas cron is authoritative. Retirement tracked at `.planning/todos/pending/retire-stub-handler.md`.
- Stale `public/data/{samples,ecdysis}.parquet` (pre-v3.0) likely still in S3 from old nightly runs; not consumed by current frontend. Cleanup tracked at `.planning/todos/pending/stale-public-data-cleanup.md`.
- `_dlt_pipeline_state` housekeeping is ad-hoc per pipeline (only `inaturalist` and `waba` DELETE on bootstrap; other 3 pipelines unaudited). Tracked at `.planning/todos/pending/dlt-pipeline-state-housekeeping.md`.
- `test_dbt_diff.py` retire-or-repurpose decision pending — post-v3.4 cutover both sides of the harness come from dbt, making it structurally a tautology. Anticipated by REQUIREMENTS.md VALIDATE-01 but not yet acted on. (INFO from v3.4 audit.)
- Nightly run failure notification (Healthchecks.io dead-man's switch) — natural follow-on now that nightly.sh exit semantics are settled. Tracked at `.planning/todos/pending/nightly-run-failure-notification.md`.
...
| Single-package layout (collapse `frontend/` to repo root) | Workspaces add CI/script complexity for a one-package repo; root-level `package.json` is simpler for a static site | ✓ Good — v3.1 Phase 74; CI scripts dropped `--workspace=frontend` cleanly |
| `index.html` must be templated (not passthrough) | `@11ty/eleventy-plugin-vite` short-circuits when `results.length === 0`; passthrough `index.html` does not register as an Eleventy result | ✓ Good — v3.1 Phase 74; `_pages/index.html` ships as Liquid no-op |
| Two-step publicDir pipeline (Eleventy passthrough → Vite publicDir) | Single-owner publicDir is wrong for the rename-and-rm mechanics of the Eleventy + Vite plugin | ✓ Good — v3.1 Phase 74; `public/` → `_site/public/` → `.11ty-vite/public/` → final `_site/` |
| Plugin `viteOptions` pass-through for dev (`envDir`, `optimizeDeps.exclude`) | Build pass reads `vite.config.ts`; dev pass via Eleventy plugin does NOT — must set via `viteOptions` | ✓ Good — v3.1 Phase 74; HMR + WASM resolution work in dev |
| Orphan diagnostic page at `/_scaffold-check/` (permanent fixture) | Deploy diagnostic for the multi-entry Vite pipeline; permanent so any breakage of layout chain or bee-header bundling shows up there | ✓ Good — v3.1 Phase 75 D-01 |
| Nunjucks templating (over Liquid) for new pages | Existing pnwmoths pattern; Eleventy `.njk` extension is the convention; SPA `index.html` continues as Liquid no-op | ✓ Good — v3.1 Phase 75 D-02 |
| Two-layer layout chain via Eleventy front-matter `layout:` + `{{ content \| safe }}` | NOT Nunjucks `{% extends %}`/`{% block %}` — mixing the two breaks rendering. Pnwmoths-pattern verbatim | ✓ Good — v3.1 Phase 75 D-03 |
| `<bee-header>` Lit component embedded via side-effect Vite entry | One-line `import` modules in `src/entries/` whose sole purpose is to be Rollup entries that trigger custom-element registration via `@customElement` decorators | ✓ Good — v3.1 Phase 75 D-04; pattern reusable for v3.2 species pages |
| Multi-entry Vite build via plugin's HTML processor (`appType: "mpa"`) | Vite walks every emitted templated HTML page and produces a hashed bundle automatically — no `rollupOptions.input` config needed; no manifest required | ✓ Good — v3.1 Phase 75 D-05; load-bearing assumption A5 verified end-to-end |
| `dir.{includes,layouts,data}` resolve relative to `dir.input` in Eleventy 3.x | When `dir.input` is a leaf folder (`_pages/`) but layout files live at repo root, use `"../_<name>"` traversal | ✓ Good — v3.1 Phase 75; documented inline in `eleventy.config.js` |
| Underscore-prefix-in-URL via `permalink:` (not filename) | Eleventy treats leading underscore in input filenames ambiguously; `permalink: /_scaffold-check/index.html` with non-underscore filename sidesteps the ambiguity | ✓ Good — v3.1 Phase 75 |
| `_data/build.js` default-export-an-object pattern | Build-time metadata feed; v3.2 can add `_data/species.js`, `_data/photos.js` etc. following the same shape | ✓ Good — v3.1 Phase 75; pattern established for v3.2 |
| FORMAT CSV macro retained with three-section rationale (D-03 override) | Research established FORMAT GDAL adds incompatible `name` key to FeatureCollection root and FORMAT JSON breaks bare-scalar structure; FORMAT CSV is the only DuckDB COPY path emitting raw VARCHAR verbatim | ✓ Good — v3.4 Phase 85 CLEAN-01; override accepted by user, replacement formally deferred |
| 33→30 column drop on `marts/occurrences` contract (drop `specimen_inat_login`, `specimen_inat_family`, `specimen_inat_genus`) | Frontend never reads those three columns; slimming the contract reduces parquet size and tightens the schema gate | ✓ Good — v3.4 Phase 85 CLEAN-02; mart contract + sqlite.ts + diff harness all updated atomically |
| `WHERE id IS NOT NULL` in `stg_inat__observations` (TEST-01) | iNat returns one dlt tombstone row with null id; filter is the simplest correct fix and is documented in staging schema.yml | ✓ Good — v3.4 Phase 85; `not_null` + `unique` tests now both pass on 10,845 rows |
| Singular SQL test for ecdysis_id reference (TEST-02) | Generic `relationships` test fails because `ecdysis_id` (BIGINT) and `catalog_number` (VARCHAR `WSDA_...`) live in different identifier namespaces; join via `stg_ecdysis__occurrences.id` is the correct relationship | ✓ Good — v3.4 Phase 85; replaces broken generic test |
| `species_export.py` is a thin dbt-mart consumer (PORT-01) | The dbt mart owns the SQL universe + aggregates; Python handles slug append (needs `feeds._slugify`) and JSON serialization (byte-stability via deterministic key ordering) | ✓ Good — v3.4 Phase 86; reads 18-col sandbox parquet, writes 19-col public parquet + species.json + seasonality.json |
| Ingestion-boundary doc codifies SQL-vs-Python split (PORT-02, PORT-04) | HTTP API calls, rate limiting, UPSERT semantics, and CSV side effects are Python-shaped; pure table transforms are SQL-shaped. Document at `.planning/phases/086-port-remaining-transforms/ingestion-boundary.md` is the durable contract | ✓ Good — v3.4 Phase 86; `resolve_taxon_ids.py` and `ecdysis_pipeline.load_links` stay in Python; dbt consumes outputs via `source()` |
| LIN-05 lineage coverage enforced via dbt singular test (PORT-03) | `test_lin05_lineage_coverage.sql` asserts ≥0.95 ratio via `ref()` (not `source()`) so it composes with dbt's DAG and re-runs on every `dbt build`; production data shows 100% coverage | ✓ Good — v3.4 Phase 86 |
| KEEP FULL REBUILDS — no `materialized='incremental'` anywhere (TEST-03) | dbt-duckdb 1.10.1 does not support incremental + external materializations (upstream README + issue #74 open since 2022); experimental run on `int_combined` showed wall-clock savings well under 30% threshold; ARM 2 (provisional WABA rows with NULL `ecdysis_id`) introduces a UNION-skip complexity tax; downstream `marts/occurrences` external mart rebuilds fully regardless | ✓ Good — v3.4 Phase 87; recommendation locked in `087-FINDINGS.md` |
| `_run_dbt_build` in `run.py` calls subprocess + copies 3 artifacts (CUTOVER-01) | dbt produces parquet/GeoJSON to `target/sandbox/` via external materializations; the Python step copies `occurrences.parquet, counties.geojson, ecoregions.geojson` to `EXPORT_DIR` (default `public/data/`). `species.parquet` is the exception: `species_export.py` reads it from sandbox and writes the slug-appended 19-col version directly | ✓ Good — v3.4 Phase 88; `subprocess.run(check=True)` propagates dbt's documented 0/1/2 exit codes through `main()`'s exception handler |
| `_apply_migrations` retired; invariants live in dbt source contracts (CUTOVER-02) | Both migrations (Phase 48 `host_observation_id` rename, Phase 47 `geom GEOMETRY` backfill) are already applied to the prod DuckDB; dbt `sources.yml` declarations + staging models now Binder-Error at compile time on any regression, which is a stronger gate than the runtime-only `_apply_migrations` check | ✓ Good — v3.4 Phase 88; mapping table preserved in `088-CUTOVER-LOG.md` |
| dbt 30-column contract on `marts/occurrences` replaces `scripts/validate-schema.mjs` (CUTOVER-03) | dbt's contract is enforced at build time (compile-time Binder Error on contract violation) and runs on every `dbt build` invocation; the JS validator was a build-time-only runtime check that ran after dbt and was redundant | ✓ Good — v3.4 Phase 88; CLAUDE.md updated to name dbt-contract as the schema gate |
| `data/dbt/run.sh` is version-pinned; do NOT modify | Wrapper invokes `uvx --from dbt-core==1.10.1 --with dbt-duckdb==1.10.1 dbt`; touching it invalidates reproducibility | ✓ Good — v3.4 explicit constraint across all phases; verified by negative grep in plan-checker |
| Pre-cutover SHA captured as rollback marker (`pre-cutover-sha.txt`) | Single-commit rollback target for the entire v3.4 milestone; mirrors the Phase 87 pre-experiment-sha pattern | ✓ Good — v3.4 Phase 88; `44a967c` is the pre-cutover HEAD |
