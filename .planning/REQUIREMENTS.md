...
- [x] **PAGE-02**: Subfamily pages are generated (SVG map + specimen/observation counts + attribution) consistent with the existing genus/subgenus/tribe pages.
- [x] **PAGE-03**: Page generation keys on `taxon_id` internally; public slugs stay name-based; same-named distinct taxa (e.g. genus vs. subgenus *Bombus*) never collapse, and any genuine slug collision is resolved deterministically.
