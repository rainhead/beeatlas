...
- [x] **PIPE-01**: Pipeline produces `inat_obs.parquet` from a committed iNat CSV export file; output columns include `obs_id`, `observed_on`, `lat`, `lon`, `canonical_name`, `scientific_name`, `user_login`, `image_url`, `license`, `floral_host`
...
- [x] **PIPE-03**: Rows whose `id` matches an existing `specimen_observation_id` in the Ecdysis dbt model are excluded from `inat_obs.parquet` (deduplicate against Ecdysis-linked obs)
