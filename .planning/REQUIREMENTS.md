...
- [x] **GEN-01**: Genus page lists all species belonging to that genus with specimen counts
- [x] **GEN-02**: Genus page displays a static SVG occurrence map with each species rendered in a distinct color
- [x] **GEN-03**: Each species entry on the genus page links to its individual species page
...
- [x] **SPE-01**: Each species in the WA checklist has a dedicated static page at `/species/{Genus}/{specificEpithet}/`
- [x] **SPE-02**: Species page displays photo(s) from `content/species-photos.toml` (fallback when none)
- [x] **SPE-03**: Species page displays a static SVG occurrence map for that species alone
- [x] **SPE-04**: Species page displays a seasonality visualization

### Pipeline & Build

- [x] **PIPE-01**: Eleventy generates one static page per species, genus, subgenus, and tribe from `species.json` data
- [x] **PIPE-02**: `species_maps.py` generates multi-color SVG occurrence maps for genus, subgenus, and tribe pages (each species assigned a distinct color within the group)
