...
- [ ] **WRITE-01**: A thin managed write app (a maderas-hosted Flask/WSGI service served by Waitress behind Apache `mod_proxy_http`, isolated alongside the existing pipeline — per 177 D-01 + 178 D-17, NOT `mod_fcgid`/flup6, NOT API Gateway + Lambda) accepts authenticated writes; the read path remains fully static.
