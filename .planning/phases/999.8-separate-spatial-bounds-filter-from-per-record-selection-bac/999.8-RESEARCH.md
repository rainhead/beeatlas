...
1. **`_onNearMeCleared` pane state — leave open or close?**
...
### Open Questions
1. `_onNearMeCleared` — should clearing bounds also collapse the pane? (D-04 says no pane touch; CONTEXT.md only addresses the apply path)
2. `_onClearSelection` — confirm clearing per-record selection leaves bounds untouched (D-05 implies yes)
3. `_onPaneCollapse` — confirm pane collapse leaves bounds active (D-07 implies only `near-me-cleared` clears bounds)
