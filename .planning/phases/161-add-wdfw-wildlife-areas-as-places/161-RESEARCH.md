# Phase 161: Add WDFW wildlife areas as places - Research
...
- Linear "hikes" as places (Phase 162).
...
v5.2 carries no formal REQUIREMENTS.md IDs for this phase. The phase boundary and decisions in `161-CONTEXT.md` are the requirement set. The implicit acceptance criteria the plan must satisfy:
...
**Valid until:** ~2026-07-22 (stable; the WDFW layer and place pipeline change rarely. Re-verify the overlap set at the final chosen tolerance immediately before commit, since `places.toml` may gain entries from Phase 162+.)
