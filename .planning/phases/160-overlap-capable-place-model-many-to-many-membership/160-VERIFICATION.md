...
overrides_applied: 0
human_verification:
  - test: "Open a multi-place occurrence in the sidebar detail pane and confirm ALL member place names render as readable chips (D-04)."
    expected: "An occurrence whose coordinate falls in two or more places shows every member place NAME (not slug), styled consistently with the existing place-name pattern; an occurrence in no place shows no place chips. The mounted-component unit test asserts the chips appear in the shadow DOM, but visual placement/styling in the live sidebar cannot be confirmed programmatically."
    why_human: "Visual appearance and sidebar layout are UX behaviors grep/unit tests cannot fully confirm. Current production places.toml has only 1 real multi-place occurrence (inat_obs:320276469); the dense overlap cases arrive with Phase 161 WDFW."
---
...
| Real overlap → multi-row | GROUP BY occ_id HAVING COUNT>1 | inat_obs:320276469 → 2 places | ✓ PASS |
...
**Test:** Open the sidebar occurrence detail for an occurrence that falls within two or more places (e.g. `inat_obs:320276469`, which the current bridge maps to 2 places; richer cases arrive with Phase 161 WDFW).
