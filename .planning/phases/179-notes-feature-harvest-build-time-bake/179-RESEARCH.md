...
> Both questions below were resolved via the inline recommendations and are implemented in the plans: the harvest uses plain SQLAlchemy + a `collectors.json` read (179-03 Task 1), and the read endpoint is `GET /api/notes?species=<canonical_name>` (179-02 Task 2). A3 above is also resolved — `build_time_fetch_optional=true` is required (deploy.yml hard-fails on a missing required manifest key), applied in 179-03.
