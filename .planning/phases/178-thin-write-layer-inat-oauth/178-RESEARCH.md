...
1. **Does Phase 178 need a real (non-stub) authenticated POST endpoint to satisfy the WRITE-03 security UAT ("forged-author request AND cross-origin POST both rejected")?**
...
2. **Exact PKCE + confidential-client compatibility with iNat's live Doorkeeper config** — could not be fully confirmed without a registered app + test credentials (out of scope for this research session, which has no iNat OAuth app credentials).
