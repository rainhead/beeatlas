...
The write layer is a **maderas-hosted Flask (WSGI) app** — the shape locked by 177's **D-01** and already reflected in `data/notes_app/main.py`. **⚠ Serving mechanism revised 2026-07-03 — see D-17: the app is served via a maintained pure-Python WSGI server (Waitress) behind Apache `mod_proxy_http`, NOT `mod_fcgid`/`flup6`.** The `flup6` FastCGI bridge was rejected (last released 2015 — an unmaintained decade-stale dependency is not acceptable). References to `mod_fcgid`/`flup6`/`.fcgi` in D-04, D-11, the code-context notes, RESEARCH.md, and plans 178-01/03/06/08 are superseded by D-17. The ROADMAP's Phase-178 text and WRITE-01/WRITE-02 requirement wording still describe the **rejected** "API Gateway HTTP API + Lambda / event-driven / within the CDK stack / short-lived session" shape. That is stale AWS framing from before the D-01 pivot. **A ROADMAP/REQUIREMENTS re-scope edit is a pre-planning action** (see Deferred Ideas) — the maderas-Flask shape is authoritative.
...
- **D-04:** **One long-lived, stateless signed cookie.** `HttpOnly` + `Secure` + `SameSite` cookie carrying a signed (itsdangerous or JWT) payload `{internal user id, iNat identity, role, long expiry — e.g. weeks}`. No server-side session store — the stateless cookie was originally motivated by `mod_fcgid`'s ephemeral workers, and remains the right call under D-17/Waitress (avoids per-process session affinity and keeps the app trivially restartable/reloadable). Rationale: "no great threat; don't make people log in often."
...
- **D-11:** **Serving topology = the `api.beeatlas.net` subdomain** (as `178-RESEARCH.md` designed): a direct Route53 A-record to maderas (`45.79.96.48`) + certbot TLS, with the Flask app behind Apache. **(⚠ The "behind Apache `mod_fcgid`" mechanism is SUPERSEDED by D-17 — Waitress behind `mod_proxy_http`. The subdomain/DNS/TLS/CORS topology below is unchanged.)** The static `beeatlas.net` site calls it **cross-origin but same-site** (shared registrable domain), so CORS is real (`flask-cors` scoped to the exact `https://beeatlas.net` origin, credentials enabled, never wildcard+credentials) but `SameSite=Strict` cookies still work. CloudFront path-routing to maderas was considered and rejected.
...
### Serving mechanism — Waitress behind Apache mod_proxy (revised 2026-07-03, supersedes the mod_fcgid parts of D-11)
- **D-17:** **The WSGI app is served by [Waitress](https://docs.pylonsproject.org/projects/waitress/) — a maintained, pure-Python WSGI server — as a persistent localhost process, reverse-proxied by Apache `mod_proxy_http`.** This **replaces the rejected `mod_fcgid`+`flup6` shape** (D-11's `.fcgi` wrapper). Rationale: flup6's last release was 2015; depending on a decade-unmaintained package for the write layer's HTTP front door is unacceptable (supply-chain + the `flup` `debug=True`/`cgitb` traceback footgun). Waitress keeps flup6's one real virtue — **pure-Python, so no compiling an Apache module against the project's bleeding-edge Python 3.14** (the app's own venv runs it) — while being actively maintained (3.0.2, security fixes through 2026). Reverse-proxy is also the modern norm Flask's own docs point to (they dropped FastCGI/flup years ago).
  - **Topology unchanged from D-11:** still `api.beeatlas.net` (Route53 A-record → 45.79.96.48) + certbot TLS; Apache terminates TLS and `ProxyPass / http://127.0.0.1:<port>/` to Waitress bound to loopback only. CORS (`flask-cors` scoped to `https://beeatlas.net`, credentials, never wildcard+credentials) and `SameSite=Strict` are unchanged. Flask needs `ProxyFix` (or equivalent) to trust the `X-Forwarded-*` headers Apache sets, so it sees the real client/scheme.
  - **Alternatives considered:** *gunicorn* (equally pure-Python/maintained, but heavier worker-model tuning for a low-traffic internal API — Waitress is the leaner pick); *mod_wsgi daemon mode* (maintained and needs no external supervisor since Apache manages the daemon, but it is a **C module that must compile against Python 3.14** + a server-global `LoadModule` requiring root and an Apache restart — highest operator friction on a no-passwordless-sudo host; rejected on that friction/3.14-compile risk).
  - **Py3.14 caveat:** no WSGI server officially tests against 3.14 yet (it is new). For pure-Python Waitress this is low-risk and empirically verifiable (import + `serve()` on 3.14 — the same evidence bar RESEARCH used for flup6, now on a maintained package). Verify during 178-03/06.
- **D-18 — SUPERVISION (OPERATOR, decided 2026-07-03): confirm systemd first, then choose.** Waitress is a persistent process, so it needs a supervisor to start on boot / restart on crash. **The operator must first confirm whether maderas has systemd** (the "no systemd" claim from 177 that drove the on-demand `mod_fcgid` choice was never verified). If systemd is present → a **`--user` systemd unit** (`WantedBy=default.target`, `loginctl enable-linger`) is the cleanest supervisor. If not → a **cron `@reboot`** entry (maderas already runs cron for the nightly + hourly-backup jobs), optionally with a `flock`-guarded keepalive cron for crash restart. The 178-08 launch-gate runbook encodes both branches and records which was used.
...
### Reusable Assets
- `data/notes_app/main.py` — Flask WSGI app skeleton (moves to `api/` per D-15); extend with OAuth callback, session, and write routes. Served via Waitress + `mod_proxy` (D-17), not `mod_fcgid`.
