...
|-------|-----------|-------------------|
| Single-token (genus) bee names non-null in `occurrences.parquet` (35/36, non-bee genera excluded) | duckdb: single-token NULL-canonical rows for bee genera → 0 | `pytest tests/test_dbt_scaffold.py::test_occurrences_taxon_id_non_null -x` (re-scoped) |
| ~17k rows newly non-null; whole-column NULL count drops accordingly | duckdb `COUNT(*) FILTER(taxon_id IS NULL)` snapshot before/after | row-count duckdb snippet |
| Truly-unidentified rows (NULL canonical_name, no genus) still NULL | duckdb: `COUNT(*) WHERE canonical_name IS NULL AND genus is empty AND taxon_id IS NOT NULL` == 0 | duckdb assertion |
...
| Non-bee bycatch genera (crossocerus, larropsis, plecoptera, symphyta, trypoxylon) stay NULL (D-07) | Anthophila filter in genus staging model + test exclusion | re-scoped not_null test |
...
| Genus taxon_id correct, no plant collision | spot-check `stelis`→127831 (bee), `lasioglossum`→57678 | duckdb spot-check query |
...
- [ ] `data/dbt/models/staging/stg_inat__genus_taxon_ids.sql` — new staging model reading `../raw/taxa.csv.gz` (`rank='genus' AND active='true'`, Anthophila-ancestry disambiguated, BIGINT→INTEGER cast) — must exist before the `int_combined` edit
- [ ] Re-scope `test_occurrences_taxon_id_non_null` (D-04/D-05) + matching `schema.yml` `where:` clause
- [ ] Scope `test_taxon_id_consistency` to `canonical_name LIKE '% %'` (D-06)
- [ ] (optional) `test_genus_backfill_resolved` — assert representative genera resolve to expected bee taxon_ids (stelis→127831, lasioglossum→57678)
