...
**Phases:** 7 (167–172, incl. inserted 171.1)
...
- [x] **Phase 167: Collector Identity Column** — COALESCE `collector_inat_login` into the occurrences mart; dbt contract 36→37; data-before-code S3 release
- [x] **Phase 168: Temporal Lifecycle Dates** — Surface intrinsic lifecycle dates (collection, posting, identification) into the mart; dbt contract bump; waba_specimen→ecdysis transition linkage; data-before-code S3 release
- [x] **Phase 169: Per-Collector Static Pages** — Export `collectors.json`, generate Eleventy pages at `/collectors/{login}/` following the places pattern
- [x] **Phase 170: Source → Provenance Facets Rebuild** — Replace the `source` enum with orthogonal provenance-tier facets across all three coupled consumers; atomic commit with positional-coupling assertion
- [x] **Phase 171: Per-Collector Event Stream** — Reverse-chronological collection→ID feed on the collector page; waba_specimen cataloguing event; pagination
- [ ] **Phase 171.1: Collector Data Delivery Rebuild** (INSERTED) — Move `collectors.json` + `collector_event_pages.json` off git onto the S3 + `manifest.json` + `deploy.yml` fetch pattern; purge from history; define page freshness mechanism. Fixes a Phase 171 design defect.
- [x] **Phase 172: Accomplishment View** — County coverage SVG map, taxonomic-breadth list, ecoregion breadth, active-seasons badge on the collector page (completed 2026-06-28 — verification 5/5, operator UAT PASS)
...
|-------|----------------|--------|-----------|
| 167. Collector Identity Column | 1/1 | Complete | 2026-06-25 |
| 168. Temporal Lifecycle Dates | 1/1 | Complete | 2026-06-25 |
| 169. Per-Collector Static Pages | — | Complete | 2026-06-25 |
| 170. Source → Provenance Facets Rebuild | 2/2 | Complete | 2026-06-27 |
| 171. Per-Collector Event Stream | 3/3 | Complete | 2026-06-27 |
| 171.1. Collector Data Delivery Rebuild (INSERTED) | 0/TBD | Not started | - |
| 172. Accomplishment View | 5/5 | Complete | 2026-06-28 |
...
**Goal**: Collector page data (`collectors.json`, `collector_event_pages.json`) is delivered like every other build-time data artifact — uploaded to S3 by the nightly (content-hashed, in `manifest.json`) and fetched at build by `deploy.yml` — instead of committed to git. The 29 MB+ artifact and its history are removed from the repo, and the static collector pages get a defined freshness mechanism.
**Depends on**: Phase 171 (created the artifacts and the defective committed-to-git delivery)
**Blocks**: Phase 172 (extends the same `collectors.json` delivery)
**Why**: Phase 171 committed a 29 MB daily-regenerated artifact to git via a `.gitignore` allowlist, deviating from the `species.json` S3 pattern — ~150 MB history bloat, and live pages stale because the nightly regenerates but never commits/redeploys. See memory `feedback_no_committed_data_artifacts`.
**Success Criteria** (what must be TRUE):
  1. nightly uploads both files to S3 (hashed, in `manifest.json`); `deploy.yml` fetches them; clean-checkout `npm run build` + `npm test` pass
  2. Both files untracked + purged from git history; `.gitignore` allowlist exceptions removed; repo size drops
  3. Static collector pages refresh **daily**: `nightly.sh` triggers a site rebuild/redeploy (`repository_dispatch` to the deploy workflow) after uploading the artifacts — pages go live nightly without a code push
  4. Live collector pages reflect current data (carries the undetermined-events fix already in `main`)
**Plans**: TBD
**Resolved decision (2026-06-28)**: freshness = nightly-triggered rebuild via `repository_dispatch` (deploy workflow accepts the event; `nightly.sh` fires it — needs a GitHub auth path on maderas).
