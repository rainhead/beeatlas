...
**Milestone Goal:** Generate Atom feeds of recent determinations (by collector, genus, county, ecoregion) from the existing DuckDB data and publish to S3 on every nightly run.
...
**Goal**: Generate a valid Atom feed of recent determinations from the existing DuckDB data
**Depends on**: Phase 41
**Requirements**: FEED-01, FEED-02, FEED-03, FEED-04, PIPE-01
**Success Criteria** (what must be TRUE):
  1. feeds.py generates valid Atom XML with entries for recent determinations
  2. Each entry contains taxon name, determiner, ecdysis link, collector, and collection date
  3. Entries are limited to 90-day window on modified timestamp, sorted newest-first
  4. Output file written to frontend/public/data/feeds/determinations.xml
**Plans**: 1 plan
Plans:
- [x] 42-01-PLAN.md — Atom feed generator module (feeds.py) wired into run.py

### Phase 43: Feed Variants
**Goal**: Generate per-collector, per-genus, per-county, and per-ecoregion feed variants plus an index
**Depends on**: Phase 42
**Requirements**: FEED-05, FEED-06, FEED-07, FEED-08, PIPE-03
**Success Criteria** (what must be TRUE):
  1. One collector-{slug}.xml, genus-{slug}.xml, county-{slug}.xml, and ecoregion-{slug}.xml per unique value
  2. Empty variant feeds (0 matching entries) are written as valid Atom with 0 entries
  3. index.json lists every variant feed with title, filter_type, and entry_count
**Plans**: 1 plan
Plans:
- [x] 43-01-PLAN.md — Variant feed generation and index.json
...
**Success Criteria** (what must be TRUE):
  1. nightly.sh uploads feed XML files to S3 alongside parquet/geojson exports
  2. Browsers visiting the site can autodiscover the determinations Atom feed
**Plans**: 1 plan
Plans:
- [x] 44-01-PLAN.md — nightly.sh S3 upload + HTML autodiscovery link tag
