...
- [x] **Phase 16: Pipeline Spatial Join** — Specimens and samples get county and ecoregion_l3 columns at build time; GeoJSON boundary files simplified, bundled with the frontend build, and validated by CI schema checks (completed 2026-03-14)
...
- 🚧 **Phase 18: Map Integration** — Region boundary overlay toggle wired into map; polygon click adds region to active filter; region filter state encoded in URL (gap closure in progress)
- [x] **Phase 19: Sidebar UI** — County and ecoregion multi-select chips in sidebar; boundary toggle control; clear-all resets region filters (completed 2026-03-18)
...
**Plans**: 2 plans
Plans:
- [x] 17-01-PLAN.md — parquet.ts + filter.ts: Parquet region columns and FilterState region Sets
- [x] 17-02-PLAN.md — region-layer.ts: GeoJSON-backed VectorLayer + browser verify checkpoint
...
**Plans**: 4 plans
Plans:
- [ ] 18-01-PLAN.md — bee-map.ts: boundaryMode state, regionLayer wiring, URL encode/decode for bm=/counties=/ecor=
- [ ] 18-02-PLAN.md — bee-map.ts: polygon singleclick, floating toggle UI, sample ghosting + human verify
- [ ] 18-03-PLAN.md — gap closure: regenerate ecdysis.parquet + samples.parquet with county/ecoregion_l3 columns
- [ ] 18-04-PLAN.md — gap closure: selected polygon highlight + single-select/shift-click behavior
