...
**Requirements**: PWK-01, PWK-02, PWK-03
**Success Criteria** (what must be TRUE):

  1. `test_dbt_diff.py` docstring updated to reflect actual 36-column `occurrences` contract (not stale "30 cols")
...
**Goal**: All species with occurrence_count > 0 appear in species.json and the species tree, regardless of WA checklist membership; each has a static page and SVG occurrence map
**Depends on**: Phase 124
**Requirements**: SPV-01, SPV-02, SPV-03
**Success Criteria** (what must be TRUE):

  1. All species with `occurrence_count > 0` appear in `species.json` and the species tree with a valid `specific_epithet`, regardless of WA checklist membership
  2. Each newly-visible species has a static page at `/species/{Genus}/{specificEpithet}/`
  3. Each newly-visible species has an SVG occurrence map generated by `species_maps.py` and uploaded to S3
...
**Goal**: species.parquet and occurrences.parquet each carry a non-null taxon_id INTEGER column; species pages link to iNat taxa
**Depends on**: Phase 125
**Requirements**: TID-01, TID-02, TID-03
**Success Criteria** (what must be TRUE):

  1. `species.parquet` includes a non-null `taxon_id` INTEGER column identifying the iNat taxon for every species row
...
**Goal**: Pipeline detects inactive taxon IDs, auto-generates remappings via iNat API where possible, writes unresolvable names to report file; manual entries take precedence
**Depends on**: Phase 126
**Requirements**: ITR-01, ITR-02, ITR-03, ITR-04
**Success Criteria** (what must be TRUE):

  1. Pipeline detects canonical names whose resolved iNat taxon ID is inactive; automatically generates remapping entries for those with a known current synonym (via iNat API `current_synonymous_taxon_ids`)
  2. Canonical names with inactive taxon IDs and no resolvable current synonym are written to `data/inactive_unresolved.csv` for manual triage
