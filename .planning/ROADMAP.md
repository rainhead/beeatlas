...
- ✅ **v7.0 Species Trait Annotations** — Phases 173–174 (shipped 2026-06-30). Curated ecological traits (sociality, diet breadth + host plant, nesting, native status, cuckoo host bee) from license-clean seeds (Bee-Gap PD, Fowler specialists, genus backbone) assembled into a `species_traits` dbt mart with per-label provenance, and surfaced on the species detail page (Traits fact-sheet + linked clepto hosts) and the index/genus/subgenus pages (sociality + Specialist badges) via the Path-B `species.json` merge. Merged via PR #39. See [.planning/milestones/v7.0-ROADMAP.md](milestones/v7.0-ROADMAP.md).
- 🚧 **v8.0 Authoritative Data Foundation** — Phases 176–180 (in progress, started 2026-07-02). Introduces BeeAtlas's first *authoritative, non-reproducible* data — WA-specific expert species natural-history notes with no iNat/Ecdysis upstream — and refounds the build seam around an explicit **derived-vs-authoritative** split so both data classes are managed correctly. Build-seam refoundation (a single declarative `data/artifacts.toml`) lands first; each authoritative capability (store + backup → write layer + iNat OAuth → notes bake → moderation) is provable before the next builds on it, and public writes are gated on a demonstrated backup restore. Requirements: SEAM / STORE / WRITE / NOTES / MOD in [.planning/REQUIREMENTS.md](REQUIREMENTS.md).

## Phases

### 🚧 v8.0 Authoritative Data Foundation (Phases 176–180) — IN PROGRESS

**Goal:** Introduce BeeAtlas's first *authoritative, non-reproducible* data — WA-specific expert species natural-history notes with no iNat/Ecdysis upstream — and refound the build seam around a **derived-vs-authoritative** split so both data classes are managed correctly. The user-visible feature is a deliberately thin vertical slice; the milestone's weight is the architecture. Build-seam cleanup lands first, the runtime-bending write layer is isolated in one deployable, and public writes are gated on a proven backup restore.
...
| 180. Moderation Loop | 5/5 | Complete    | 2026-07-05 |
...
- [x] 176-03-PLAN.md — Wave 2: rewire `deploy.yml` build-time fetch via build-time-fetch verb (bare-CI python3, species_hosts tolerate-absence preserved) + ADR `docs/adr/0002-derived-vs-authoritative-artifacts.md` (two regimes + stable-dir exclusion) + CLAUDE.md pointer [SEAM-03, SEAM-05]
...
- [x] 177-01-PLAN.md — Dependencies (alembic/sqlalchemy/fastapi/uvicorn behind a package-legitimacy gate) + `notes_store`/`notes_app` scaffold + relax REQUIREMENTS.md STORE-03 wording to snapshot-based (D-11) [STORE-01, STORE-02, STORE-03]
...
- [x] 177-06-PLAN.md — Consistent-snapshot backup script (stdlib online-backup API → dedicated S3 bucket) + restore-roundtrip tests + DR runbook [STORE-03]
...
- [x] 178-01-PLAN.md — Package legitimacy checkpoint (waitress + flask-cors on PyPI) before install — blocking-human (D-17: waitress replaces the rejected flup6) [WRITE-01]
...
**Plans**: 5 plans (3 waves)
  ...
  - [x] 180-05-PLAN.md — Wave 3 (autonomous:false): operator applies migration 0004 on maderas + BLOCKING end-to-end MOD-04 human UAT (submit→publish→takedown→curl-restore); do NOT auto-advance
...
- [x] **Phase 163: Ecdysis download requires an authenticated session — add Symbiota login** — ✅ **RESOLVED (completed 2026-06-24 — nightly UNBLOCKED).** Ecdysis/Symbiota's `downloadhandler.php` returned `401 {"error":"Unauthorized access"}` (anonymous public-download path closed). Fixed with an authenticated Symbiota session (`POST /profile/index.php` real credential form `login`/`password`/`action=login`/`remember`; `PHPSESSID` on a reused `requests.Session`), a magic-bytes (`PK\x03\x04`) + Content-Type response guard before any cache write, and a cache-fallback so an outage degrades gracefully (a valid cached ZIP is reused with a warn). Creds in gitignored `data/.dlt/secrets.toml` on maderas. **Operator-verified in production:** a real authenticated download fetched a valid `44.zip` and `run.py` ran through `generate-sqlite` (the proximate UAT blocker turned out to also be a stale Symbiota password, which the operator reset; the auth-session fix remains the durable solution since the anonymous path is gone). 7 fast-tier tests; VERIFICATION PASSED; REVIEW + SECURITY done. Commits `fdfb89e1`/`20dc77ea`/`38aa6e36`. Promoted from backlog 999.12. **Depends on:** none structural. **Plans:** 9/9 plans complete
  Plans:
...
<details>
<summary>✅ v6.0 My Work — Progress & Provenance (Phases 167–172 incl. 171.1) — SHIPPED 2026-06-28</summary>
...
- [x] **Phase 168: Temporal Lifecycle Dates** — Surface the intrinsic **identification date** into the occurrences mart as a single new `id_date VARCHAR` column so a specimen history reads as a two-event timeline: **Collected** (the existing `date` column) and **Identified** (`id_date`). Scope narrowed by operator decisions (168-CONTEXT D-01..D-13): `posted_date`/iNat `created_at` is **dropped** (not a column, not an event — supersedes the roadmap's "posting" wording); collection is served by the pre-existing `date` column (no `collection_date`); the only column added is `id_date`, parsed from the dirty Ecdysis `date_identified` (keep year-only + full dates verbatim, NULL the garbage). dbt contract **37→38**; one warn-severity singular parse-completeness test; data-before-code S3 release, sequenced after Phase 167's 37-col landing (do not stack two unreleased contract bumps). TEMP-02 (waba_specimen→ecdysis = one specimen, not delete+create) is satisfied by the existing ARM-3 de-dup — no new plumbing. Data-layer only; the feed UI is Phase 171. **Depends on:** Phase 167. **Plans:** 1 plan (1 wave). (completed 2026-06-25)
  Plans:
...
- [x] **Phase 171.1: Collector Data Delivery Rebuild** (INSERTED) — Moved `collectors.json` + `collector_event_pages.json` off git onto the S3 + `manifest.json` + `deploy.yml` fetch pattern (the `species.json` pattern); committed tiny synthetic fixtures outside `public/data/` + `existsSync` guard in `_data/collectors.js` so `npm test` + `npm run build` are green on a clean checkout with zero S3 access. Fixes the Phase 171 committed-artifact defect. **Plans:** 4. (completed 2026-06-28)
...
- [x] 172-03-PLAN.md — Wave 1: collector_maps.py county + ecoregion coverage SVG generator
...
<details>
<summary>✅ v7.0 Species Trait Annotations (Phases 173–174) — SHIPPED 2026-06-30 (PR #39)</summary>
