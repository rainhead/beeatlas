...
- **Expert** — a person whose identifications assert and veto determination trust ([ADR 0033](docs/adr/0033-trust-an-expert-unless-an-expert-disagrees.md)). Status lives in the curated identifier register (`data/dbt/seeds/identifier_register.csv`), which is the single authority — the expert-feed loader (`data/inat_expert_pipeline.py`) and the photo-pipeline trust gate both derive their rosters from it. Ecdysis determiners need no expert flag — determinations there are trusted by ingestion provenance.
...
- **Place** (data model) vs **Region** (map-UI label) — same thing, two names. Every place is a row in `geographies.places` and gets the same machinery: bridge membership, a `/places/<slug>.html` page, an SVG map, a `place=` filter.
- **Place kind** — `site` or `ecoregion_l4`, the one axis on which places differ. A **site** is a hand-curated collecting location in `content/places.toml` with a land owner and permits (WDFW wildlife areas, WTA hike corridors — linear features → ~250m buffer — and so on). An **ecoregion_l4** is an EPA Level IV ecoregion, loaded from the EPA's per-state file and carrying no land owner: nobody owns an ecoregion. Sites are 181 small polygons; the 57 Level IV ecoregions tile the whole state, so nearly every occurrence belongs to one. See [ADR 0035](docs/adr/0035-level-iv-ecoregions-are-places.md).
- **Ecoregion Level III vs Level IV** — Level III (9 in WA, `occurrences.ecoregion_l3`) is a scalar column and its own `FilterState` dimension, multi-select. Level IV (57 in WA) is a *place*, filtered singly through the bridge like any other place. They are different mechanisms for a reason — see the ADR — so "filter by ecoregion" is ambiguous; say which level.
