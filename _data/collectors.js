...
const collectorsPath = join(dataDir, 'collectors.events.json');
const collectorsArray = existsSync(collectorsPath)
  ? JSON.parse(readFileSync(collectorsPath, 'utf8'))
  : (console.warn(`[collectors.js] ${collectorsPath} absent — returning [] (fetch from S3 for full data)`), []);
...
// than ENOENT-crash.
const collectorEventPagesPath = join(dataDir, 'collector_event_pages.json');
...
if (process.env.ELEVENTY_RUN_MODE === 'build') {
  if (existsSync(collectorEventPagesPath)) {
    ...
    console.warn(`[collectors.js] ${collectorEventPagesPath} absent — returning [] (fetch from S3 for full data)`);
  }
}
